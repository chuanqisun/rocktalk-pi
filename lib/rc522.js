...
function isExpectedNoTagMessage(message) {
  return (
    message.startsWith("Timeout waiting for tag") ||
    message.startsWith("No tag in field") ||
    message.startsWith("Anticollision failed") ||
    message === "UID BCC check failed" ||
    message.startsWith("SELECT failed") ||
    message === "Cascade bit set in SAK1 but CT marker missing in CL1 response"
  );
}
...
function assertValidUserPage(page) {
  ...
  if (page < NTAG213.USER_START_PAGE || page > NTAG213_USER_END_PAGE) {
    throw new Error(`NTAG213 page ${page} is outside the user memory range ${NTAG213.USER_START_PAGE}..${NTAG213_USER_END_PAGE}`);
  }
}
...
export default class Rc522 {
  ...
  async writeTextAsync(text, options = {}) {
    ...
    return this.#withCard(settings, async ({ card }) => {
      ...
      if (!verified.equals(payload)) {
        throw new Error(`Verification failed for pages ${settings.blocks[0]}..${settings.blocks[settings.blocks.length - 1]}`);
      }
  ...
  async #withCard(settings, operation) {
    ...
    while (true) {
      try {
        ...
        if (!isExpectedNoTagMessage(message)) {
          throw error;
        }
  ...
  #transceive(data, validBits = 0, timeoutMs = this.#options.requestTimeoutMs) {
    ...
    if (timedOut) {
      ...
      this.#debugLog(`Transceive timeout data=${bytesToHex(data)} bits=${validBits} ${this.#formatDebugState(error.debugState ?? {})}`);
    ...
    if (errorValue & fatalMask) {
      ...
      this.#debugLog(`Transceive error data=${bytesToHex(data)} bits=${validBits} ${this.#formatDebugState(error.debugState ?? {})}`);
  ...
  #request(command) {
    ...
    if (response.bits !== 16 || response.data.length < 2) {
      throw new Error(`No tag in field (${command === PICC.WUPA ? "WUPA" : "REQA"} returned ${response.bits} bits/${response.data.length} bytes)`);
    }
  ...
  async #writeChunkRobust(startPage, payload, uid, attempts) {
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      this.#debugLog(`Chunk attempt ${attempt + 1}/${attempts} pages=${startPage}..${startPage + pageCount - 1} data=${bytesToHex(payload)}`);
      ...
      try {
        ...
        if (error instanceof Error && "debugState" in error && error.debugState) {
          this.#debugLog(
            `Chunk pre-read failed pages=${startPage}..${startPage + pageCount - 1} ` + `reason=${error.message} ${this.#formatDebugState(error.debugState)}`
          );
        }
      }
      ...
      for (let index = 0; index < pageCount; index += 1) {
        ...
        try {
          ...
          if (error instanceof Error && "debugState" in error && error.debugState) {
            this.#debugLog(`WRITE failed page=${page} reason=${error.message} ${this.#formatDebugState(error.debugState)}`);
          }
      ...
      const verification = await this.#waitForChunkData(startPage, payload, uid, this.#options.verifyTimeoutMs + attempt * 20);
