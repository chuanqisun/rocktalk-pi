...
function createWriteAttemptEvent({ status, page, attempt, bytes, error = null }) {
  return {
    status,
    page,
    attempt,
...
export default class Rc522 {
  ...
  #resolveRuntimeOptions(options) {
    return {
      ...
      onWriteAttempt: typeof options.onWriteAttempt === "function" ? options.onWriteAttempt : null,
    };
  }

  #emitWriteAttempt(settings, event) {
    if (!settings.onWriteAttempt) {
      return;
    }

    settings.onWriteAttempt(event);
  }
  ...
  async #writePageCloneFriendly(page, bytes, settings) {
    ...
    while (true) {
      ...
      try {
        ...
        this.#emitWriteAttempt(settings, createWriteAttemptEvent({
          status: "success",
          page,
          attempt,
          bytes,
