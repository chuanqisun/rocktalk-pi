  ...
  async writeTextAsync(text, options = {}) {
    ...
    return this.#withCard(settings, async ({ card }) => {
      ...
      for (let index = 0; index < settings.blocks.length; index += NTAG213.READ_CHUNK_PAGE_COUNT) {
        ...
        writtenChunks.push(await this.#writeChunkRobust(startPage, chunk, card.uid, settings.writeAttempts));
      }
      ...
      return {
        ...
        size: card.sak,
        data: written,
        text: decodeText(trimTrailingNullBytes(written)),
      };
    });
  }
