...
const NTAG213_TEXT_READ_CHUNK_PAGES = 4;
...
export default class Rc522 {
  ...
  async readTextAsync(options = {}) {
    ...
    return this.#withCard({ ...options, block: blocks[0] }, async ({ uid, size }) => {
      const data = this.#readTextPages(blocks[0], blocks.length);
  ...
  #readPages(startPage, pageCount) {
    ...
    for (let index = 0; index < pageCount; index += NTAG213_FAST_READ_MAX_PAGES) {
      const chunkStartPage = startPage + index;
      const chunkPageCount = Math.min(pageCount - index, NTAG213_FAST_READ_MAX_PAGES);
      const chunkEndPage = chunkStartPage + chunkPageCount - 1;

      assertValidUserPage(chunkStartPage);
      assertValidUserPage(chunkEndPage);

      const data = this.#fastReadPages(chunkStartPage, chunkEndPage);
  ...
  #readTextPages(startPage, pageCount) {
    const result = [];

    for (let index = 0; index < pageCount; index += NTAG213_TEXT_READ_CHUNK_PAGES) {
      const chunkStartPage = startPage + index;
      const chunkPageCount = Math.min(pageCount - index, NTAG213_TEXT_READ_CHUNK_PAGES);
      const chunkEndPage = chunkStartPage + chunkPageCount - 1;
      ...
      for (let pageIndex = 0; pageIndex < chunkPageCount; pageIndex += 1) {
        const pageOffset = pageIndex * NTAG213_PAGE_SIZE;
        const pageData = data.slice(pageOffset, pageOffset + NTAG213_PAGE_SIZE);

        if (pageData.every((value) => value === 0x00)) {
          return result;
        }
  ...
  #readPagesRaw(page) {
    ...
    const expectedLengths = new Set([NTAG213_READ_RESPONSE_LENGTH, NTAG213_READ_RESPONSE_LENGTH + CRC_A_BYTE_LENGTH]);

    if (result.status !== STATUS.OK || !expectedLengths.has(result.data.length)) {
      return null;
    }

    return result.data.slice(0, NTAG213_READ_RESPONSE_LENGTH);
  }

  #fastReadPages(startPage, endPage) {
    const expectedLength = (endPage - startPage + 1) * NTAG213_PAGE_SIZE;
    const buffer = [PICC.FAST_READ, startPage, endPage];
    const crc = this.#calculateCRC(buffer);
    buffer.push(crc[0], crc[1]);
