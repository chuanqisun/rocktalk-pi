...
function normalizeBlockData(data) {
  if (typeof data === "string") {
    ...
    if (encoded.length > NTAG213_PAGE_SIZE) {
      throw new Error("NTAG213 page writes cannot exceed 4 UTF-8 bytes");
    }

    const buffer = Buffer.alloc(NTAG213_PAGE_SIZE, 0x00);
    encoded.copy(buffer, 0, 0, NTAG213_PAGE_SIZE);
    return buffer;
  }
  ...
  if (Array.isArray(data) || Buffer.isBuffer(data) || ArrayBuffer.isView(data)) {
    ...
    if (buffer.length !== NTAG213_PAGE_SIZE) {
...
export default class Rc522 {
  ...
  async writeAsync(data, options = {}) {
    ...
    return this.#withCard(options, async ({ block, uid, uidBytes, size }) => {
      await this.#writePageRobust(block, payload, uidBytes, options.writeAttempts ?? this.#options.writeAttempts);
  ...
  async writeTextAsync(text, options = {}) {
    ...
    return this.#withCard({ ...options, block: blocks[0] }, async ({ uid, uidBytes, size }) => {
      ...
      for (let index = 0; index < blocks.length; index += 1) {
        ...
        const chunk = payload.subarray(index * NTAG213_PAGE_SIZE, (index + 1) * NTAG213_PAGE_SIZE);
        await this.#writePageRobust(block, chunk, uidBytes, options.writeAttempts ?? this.#options.writeAttempts);
      }
  ...
  #writePageOnce(page, data4) {
    ...
    const crc = this.#calculateCRC(frame);
    const result = this.#transceive([...frame, crc[0], crc[1]], 0x00, 60);

    if (result.bits !== 4 || result.data.length < 1) {
      return { ok: false, reason: `Bad ACK frame (${result.bits} bits)` };
    }
  ...
  async #writePageRobust(page, data4, uidBytes, attempts) {
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      ...
      try {
        const recoveredCard = this.#readCard(PICC.WUPA);

        if (uidBytes && !arraysEqual(recoveredCard.uidBytes, uidBytes)) {
          throw new Error("Different NTAG213 tag detected during post-write verification");
        }
